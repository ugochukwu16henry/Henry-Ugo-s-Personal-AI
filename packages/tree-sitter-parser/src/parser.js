...
export class TreeSitterParser {
    ...
    getSymbolPatterns(language) {
        const patterns = [];
        switch (language) {
            case 'typescript':
            case 'javascript':
                patterns.push({ regex: /(?:export\s+)?(?:async\s+)?function\s+(\w+)/, type: 'function' }, { regex: /(?:export\s+)?class\s+(\w+)/, type: 'class' }, { regex: /(?:export\s+)?interface\s+(\w+)/, type: 'interface' }, { regex: /(?:export\s+)?type\s+(\w+)/, type: 'type' }, { regex: /const\s+(\w+)\s*=/, type: 'variable' }, { regex: /(\w+)\s*\(/, type: 'method' });
                break;
            case 'python':
                patterns.push({ regex: /def\s+(\w+)/, type: 'function' }, { regex: /class\s+(\w+)/, type: 'class' }, { regex: /(\w+)\s*=/, type: 'variable' });
                break;
            case 'rust':
                patterns.push({ regex: /fn\s+(\w+)/, type: 'function' }, { regex: /struct\s+(\w+)/, type: 'class' }, { regex: /impl\s+(\w+)/, type: 'class' });
                break;
        }
        return patterns;
    }
